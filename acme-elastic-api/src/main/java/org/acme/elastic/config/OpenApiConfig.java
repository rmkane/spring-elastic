package org.acme.elastic.config;
...
public class OpenApiConfig {
    ...
    /**
     * Stable path order in Swagger UI: list → reads → writes → destructive;
     * Documents before Products; unknown paths last (alpha).
     */
    ...
    @Bean
    public OpenAPI springElasticApiOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Spring Elastic API")
                        .description(
                                "REST API for documents and products in Elasticsearch. "
                                        + "Run Elasticsearch locally (Makefile targets es-start / es-setup) "
                                        + "and set ES_PASSWORD if not using the Docker default.")
                        .version("0.1.0-SNAPSHOT")
                        .license(new License()
                                .name("Apache 2.0")
                                .url("https://www.apache.org/licenses/LICENSE-2.0.html")))
                .addServersItem(new Server().url("/").description("This application (see server.port)"));
    }
}
