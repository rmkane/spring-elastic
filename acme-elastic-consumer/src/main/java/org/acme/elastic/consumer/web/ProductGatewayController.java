...
public class ProductGatewayController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "All products (empty if index does not exist yet)", content = @Content(array = @ArraySchema(schema = @Schema(implementation = ProductJson.class))))
    })
    @GetMapping
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Products whose categoryIds contain any of the given ids", content = @Content(array = @ArraySchema(schema = @Schema(implementation = ProductJson.class)))),
            @ApiResponse(responseCode = "400", description = "No category ids provided")
    })
    @GetMapping("/by-category")
    public List<ProductJson> findByCategory(
            @Parameter(description = "Comma-separated category ids (match if product lists any of them)", example = "electronics,books") @RequestParam String categoryIds) {
        List<String> ids = Arrays.stream(categoryIds.split(",")).map(StringHelper::trimToNull).filter(Objects::nonNull)
                .toList();
        if (ids.isEmpty()) {
    ...
    @Operation(summary = "Map categories to product ids", description = "Returns one entry per requested category id (map keys sorted alphabetically). Each value is a sorted "
            + "distinct set of product ids whose categoryIds contain that category. Proxied to the upstream API.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Sorted category id → sorted distinct product ids (JSON arrays); empty sets if no match"),
            @ApiResponse(responseCode = "400", description = "Validation error (e.g. more than 10 ids)")
    })
    @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(type = "string"), maxItems = 10), examples = @ExampleObject(value = "[\"electronics\",\"books\"]")))
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Found", content = @Content(schema = @Schema(implementation = ProductJson.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Created (no id in body)", content = @Content(schema = @Schema(implementation = ProductJson.class))),
            @ApiResponse(responseCode = "200", description = "Updated or saved with client id", content = @Content(schema = @Schema(implementation = ProductJson.class)))
    })
    @PostMapping
