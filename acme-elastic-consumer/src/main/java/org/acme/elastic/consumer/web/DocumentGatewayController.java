...
@Tag(name = "Documents", description = "Proxied to the upstream Elasticsearch API (same paths and behavior)")
public class DocumentGatewayController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Documents in the index for the current week", content = @Content(array = @ArraySchema(schema = @Schema(implementation = DocumentJson.class))))
    })
    @GetMapping
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Documents found (omits unknown ids)", content = @Content(array = @ArraySchema(schema = @Schema(implementation = DocumentJson.class))))
    })
    @GetMapping("/by-ids")
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Matches in the current weekly index", content = @Content(array = @ArraySchema(schema = @Schema(implementation = DocumentJson.class)))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Found", content = @Content(schema = @Schema(implementation = DocumentJson.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Indexed in Elasticsearch (upstream)", content = @Content(schema = @Schema(implementation = DocumentJson.class))),
    ...
    @Operation(summary = "Purge the current weekly index", description = "Deletes the entire Elasticsearch index for documents for the current week upstream. "
